...
- **Phase 8: UI表示言語切替の実装:** View 表示文言の日本語 / English 切替機能を、合意済みの `ui-language-decisions.md` および `.github/prompts/implement-ui-language-switch.prompt.md` に沿って実装する。UI 表示言語は `output_language` とは分離し、`AppConfig.ui_language` に永続化する。実装は以下の 6 段階で進める。

  **A. Config と翻訳基盤:**
  `AppConfig` に `ui_language` を追加し、OS ロケールからの既定値決定と `ja` / `en` への正規化を導入する。`services/translation_service.py` と `resources/i18n.py` を追加し、階層キー方式・英語フォールバックを持つ翻訳基盤を作る。

  **B. Protocol と言語設定ダイアログ:**
  `view_interfaces.py` に表示言語設定と即時反映に必要な Protocol を追加する。`views/language_dialog.py` を新設し、ドロップダウンで `日本語` / `English` を選択できる専用ダイアログを実装する。`MainWindow` のメニューバーには「キャッシュ」の右に「言語 / Language」メニューを追加する。

  **C. Presenter 統合と即時反映:**
  `presenters/language_presenter.py` を新設し、`MainPresenter` に言語設定ダイアログ起動フローを統合する。`MainWindow` と `SidePanel` は言語変更時に即時更新し、`SettingsDialog` / `CacheDialog` は次回生成時に新言語を反映する。

  **D. View の静的文言置換:**
  `views/main_window.py`, `views/side_panel_view.py`, `views/settings_dialog.py`, `views/cache_dialog.py`, 必要なら `views/bookmark_panel.py` に存在するハードコード済み文字列を翻訳キー参照へ移行する。対象にはメニュー、アクション、ラベル、ボタン、タブ名、プレースホルダー、HTML 初期文言、空状態文言を含める。

  **E. Presenter 文言の翻訳対応:**
  `presenters/main_presenter.py`, `presenters/panel_presenter.py`, `presenters/settings_presenter.py`, 必要なら `presenters/cache_presenter.py` の status message / confirm dialog / error dialog 文言を翻訳対応する。`show_error_dialog` / `show_confirm_dialog` に渡す title / message は呼び出し側で翻訳済み文字列として組み立てる。

  **F. テストと検証:**
  `ui_language` の保存・復元、旧設定ファイル互換、translation service のフォールバック、View の静的文言、Presenter 発メッセージ、`MainWindow` / `SidePanel` の即時反映をテストで担保する。手動確認では `MainWindow` / `SettingsDialog` / `CacheDialog` / `SidePanel` の両言語スクリーンショット差分を確認する。
