...
class ISidePanelView(Protocol):
    ...
    def show_ai_request_running(
        ...
        """AI request 実行中の補助 UI をサイドパネル内に表示する。

        翻訳・カスタムプロンプトの起点がサイドパネル内にあるため、
        進行中メッセージと Cancel 操作も同じ視線上に出すための API。
        """
        ...
    def clear_ai_request_running(self) -> None:
        """AI request 実行中の補助 UI をサイドパネル内から消す。"""
        ...
...
class ISettingsDialogView(Protocol):
    ...
    def set_plotly_sandbox_timeout_s(self, value: float) -> None:
        """Plotly sandbox の timeout 秒数を設定する。"""
        ...
