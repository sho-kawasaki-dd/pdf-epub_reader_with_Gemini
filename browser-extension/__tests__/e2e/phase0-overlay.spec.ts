import { chromium, expect, test, type BrowserContext, type Locator, type Page, type Worker } from '@playwright/test';
    ...
    const server = http.createServer(async (request, response) => {
      ...
      if (request.method === 'POST' && request.url === '/analyze/translate') {
        ...
        request.on('end', () => {
          ...
          const translatedText = payload.mode === 'custom_prompt'
...
async function selectFixtureTextBySelector(page: Page, selector: string): Promise<void> {
...
async function sendTabMessage<T>(worker: Worker, message: unknown): Promise<T> {
  for (let attempt = 0; attempt < 30; attempt += 1) {
    try {
      ...
      if (!messageText.includes('Receiving end does not exist') || attempt === 29) {
        throw error;
      }
...
async function setShadowCheckbox(page: Page, selector: string, checked: boolean): Promise<void> {
  await shadowLocator(page, selector).setChecked(checked);
}

async function readShadowChecked(page: Page, selector: string): Promise<boolean> {
...
test('reopens the cached overlay and supports keyboard reruns in Chromium', async () => {
  ...
  try {
    ...
    await expect.poll(async () => readShadowText(page, '.launcher-button')).toContain('Gem Read');

    await openOverlayFromPopupHelper(context, extensionId, page);
    await expect.poll(async () => readShadowText(page, '.badge')).toContain('Live Result');

    await page.keyboard.press('Alt+R');
    await expect.poll(async () => readShadowText(page, '.result-box')).toBe(`[ja] 1. ${expectedSelectionText}`);

    await fillShadowInput(page, '.custom-prompt-input', 'Explain the highlighted paragraph');
    await page.keyboard.press('Control+Enter');
    await expect.poll(async () => readShadowText(page, '.result-box')).toBe(
      '[custom] Explain the highlighted paragraph',
    );
...
test('supports ordered multi-selection batches with explanation and custom prompt reruns', async () => {
  const userDataDir = await fs.mkdtemp(path.join(os.tmpdir(), 'gem-read-extension-batch-'));
  const { server, url } = await startFixtureServer();
  const { server: apiServer, url: apiUrl, state: apiState } = await startStubApiServer();
  let context: BrowserContext | undefined;
  ...
  try {
    ...
    await expect.poll(async () => readShadowCount(page, '.session-item-remove')).toBe(2);
    await expect.poll(async () => readShadowText(page, '.batch-counter')).toContain('2/10');

    await setShadowCheckbox(page, '.session-item-image-toggle', true);
    await expect.poll(async () => readShadowChecked(page, '.session-item-image-toggle')).toBe(true);

    await fillShadowInput(page, '.custom-prompt-input', 'Summarize both paragraphs');
    await page.keyboard.press('Control+Enter');
    await expect.poll(async () => readShadowText(page, '.result-box')).toBe(
      '[custom] Summarize both paragraphs',
    );
    ...
    expect(apiState.analyzeRequests[0].selection_metadata?.items).toHaveLength(2);
    expect(apiState.analyzeRequests[1]).toMatchObject({
      mode: 'custom_prompt',
      custom_prompt: 'Summarize both paragraphs',
      text: `1. ${expectedSelectionText}\n\n2. ${secondSelectionText}`,
...
test('supports image-only rectangle sessions with custom prompt reruns', async () => {
  const userDataDir = await fs.mkdtemp(path.join(os.tmpdir(), 'gem-read-extension-rectangle-'));
  const { server, url } = await startFixtureServer();
  const { server: apiServer, url: apiUrl, state: apiState } = await startStubApiServer();
  let context: BrowserContext | undefined;
  ...
  try {
    ...
    await expect.poll(async () => readShadowCount(page, '.session-item-remove')).toBe(1);
    await expect.poll(async () => readShadowText(page, '.session-item-text')).toContain('[Image region only]');
    await expect.poll(async () => readShadowChecked(page, '.session-item-image-toggle')).toBe(true);

    await fillShadowInput(page, '.custom-prompt-input', 'Describe the selected figure');
    await clickShadow(page, '.action-custom');
    await expect.poll(async () => readShadowText(page, '.result-box')).toBe(
      '[custom] Describe the selected figure',
    );
    ...
    expect(apiState.analyzeRequests[0].selection_metadata?.items?.[0]).toMatchObject({
      source: 'free-rectangle',
      include_image: true,
      image_index: 0,
    });
